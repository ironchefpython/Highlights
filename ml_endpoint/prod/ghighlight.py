...
def extract_scenes(url, nn_model, title, video_id, image):
	get_video(url)
	cat = get_category(video_id)
	h = Highlighter("videos/"+video_id+".3gp", nn_model, cat)
	ret = {"video_url": url, "video_id": video_id, "title": title, 'image_url':image, "highlights": h.get_highlights()}
	videos.append(ret)
...
def get_videos(channelID):
	...
	for item in feed_json1["items"]:
		...
		images.append('http://img.youtube.com/vi/%s/mqdefault.jpg' % item['id']["videoId"])
	return titles, results, images
...
titles, urls, images = get_videos(channel_id)
print urls
threads = []
count = 0
for title, url, image in zip(titles, urls, images):
	if count < 5:
		thread = Thread(target=extract_scenes, args=("https://www.youtube.com/watch?v="+url, nn_model, title, url, image))
