...
class Highlighter:
    ...
    def get_highlights(self, filename, info):
        ...
        logging.info('loaded %i bytes from %s' %
                     (path.getsize(filename), filename))

        length = int(cap.get(CV_CAP_PROP_FRAME_COUNT))
        if length < 5:
            raise ValueError(
                "Video must be longer than 5 seconds, but is " + str(length))
        vid = {
            'length': length,
        ...
        q = self.highlight_part(lock, cap, vid, 0, length, Queue())
...
# highlight_part(lock, cap, vid, 0, length, Queue())
    ...
    # recursively partition, analyize, and recombine information from a video
    # file
    ...
    #   threadr.join()
        ...
        if combine(llf, rff, hl[-1]["avrg_bspeed"]):
            ...
            hl[-1]["max_bspeed"] = max(hl[-1]
                                       ["max_bspeed"], hl[0]["max_bspeed"])
            hl[-1]["max_brightness"] = max(hl[-1]
                                           ["max_brightness"], hl[0]["max_brightness"])
            hl[-1]["avrg_brightness"] = (hl[-1]
                                         ["avrg_brightness"] + hl[0]["avrg_brightness"])/2.
            hl[-1]["avrg_bspeed"] = (hl[-1]
                                     ["avrg_bspeed"] + hl[0]["avrg_bspeed"])/2.
            del hr[0]
                ...
                if i > 0 and i < 2:
                    ...
                    if diff > exp_avrg or (currentframe + self.skip_rate+1) >= end:
                        if current_frame_start != currentframe:
                            avrg_brightness /= 1.0 * \
                                (currentframe-current_frame_start) / \
                                (self.skip_rate+1)
...
def get_fps(cap):
    fps = cap.get(CV_CAP_PROP_POS_FRAMES)
